...
fifteenmax <- sort(synthdata$Population[synthdata$DateNumeric==1&
                                          synthdata$`County type`=="SK"],
                   decreasing = TRUE)[1:15] #ten cities with with highest Population numbers
donorcities<-synthdata$UnitNumeric[which(synthdata$Population%in%fifteenmax)][seq(1,15*245,by = 245)][-c(1:2)]

#Pool
pool<-synthdata[which(synthdata$UnitNumeric%in%c(3,12,donorcities)
...
MSPErestrict = 5 #only placebo controls are considered, that have an MSPE of less than five times that of the original control
  ...
  write_xlsx(data.frame(as.vector(rownames(synth.tables.orig$tab.pred)[c(4:6,2,3,1)]), #row names, the vector c(4:6,2,3,1) correct the order of rows to fit the final text
             ...
             "Figures and Tables/Table 3.xlsx") 
  # Table 4####
  write_xlsx(data.frame(synth.tables.orig$tab.w), 
             "Figures and Tables/Table 4.xlsx") 
  
  # Figure 8: treated and synthetic####
  png(filename="Figures and Tables/Figure 8 - HH Growth Rate _ Synthetic Control.png", 
      units="px", 
      width=1980, 
      height=1080, 
      pointsize=25, 
  ...
  png(filename="Figures and Tables/Figure 9 - HH Growth Rate _ Placebo Tests.png", 
      units="px", 
      width=1980, 
      height=1080, 
      pointsize=25, 
  ...
  placebolistHH[length(placebolistHH)+1] <- list(a)
  names(placebolistHH)[length(placebolistHH)] <- "original treatment unit"
  ...
  for (unit in pool$UnitNumeric) {
    ...
    names(a)<-c("Y1","Y0","synth.out","synth.tables")
    placebolistHH[length(placebolistHH)+1] <- list(a)
    names(placebolistHH)[length(placebolistHH)] <- unit
    
    #skip, if MSPE exceeds restriction
    if (synth.tables$tab.loss[2]>MSPErestrict*placebolistHH[[1]][[4]]$tab.loss[2])
    {next}
  ...
  i=1
  PostPre<-c()
  while (i<= length(placebolistHH)) { #this loop uses the data stored in placebolistHH to calculate the Post/Pre Interventions MSPEs for all synthetic controls (subject to the constraint, that the pretreatment MSPE is not 'MSPErestrict' times bigger than the original MSPE)
    PostPre[length(PostPre)+1]<-
      (sum((placebolistHH[[i]][[1]][c((1+length(optimization.period)):length(plot.period)),]- #Post-Intervention MSPE
              placebolistHH[[i]][[2]][c((1+length(optimization.period)):length(plot.period)),] %*% placebolistHH[[i]][[3]]$solution.w)^2#Post-Intervention MSPE
      )/(length(plot.period)-length(optimization.period)))/ 
      (sum((placebolistHH[[i]][[1]][1:length(optimization.period),] - #Pre-Intervention MSPE
              placebolistHH[[i]][[2]][1:length(optimization.period),] %*% placebolistHH[[i]][[3]]$solution.w)^2
      )/length(optimization.period))
    i<-i+1
  }
  
  png(filename="Figures and Tables/Figure 15 - HH Growth Rate _ PrePost MSPE.png", 
      units="px", 
      width=1080, 
      height=1080, 
      pointsize=25, 
  ...
  png(filename="Figures and Tables/Figure 16 - HH Growth Rate _ Leave-one-out Donors.png", 
      units="px", 
      width=1980, 
      height=1080, 
      pointsize=25, 
  ...
  png(filename="Figures and Tables/Figure 17 - HH Growth Rate _ Leave-one-out Predictors.png", 
      units="px", 
      width=1980, 
      height=1080, 
      pointsize=25, 
  ...
  robustnesslist2.HH.COVID<-list() #creating a list that contains all synthetic control estimations conducted for this robustness test. This list won't be used for further results, but has a purpose for double-checking results
  a<-list(dataprep.out.orig$Y1plot,dataprep.out.orig$Y0plot,synth.out.orig,synth.tables.orig)
  names(a)<-c("Y1","Y0","synth.out","synth.tables")
  robustnesslist2.HH.COVID[length(robustnesslist2.HH.COVID)+1] <- list(a)
  names(robustnesslist2.HH.COVID)[length(robustnesslist2.HH.COVID)] <- "original treatment unit"
  ...
  for (predictor.number in c(1:length(var.special.predictors))) {
    ...
    names(a)<-c("Y1","Y0","synth.out","synth.tables")
    robustnesslist2.HH.COVID[length(robustnesslist2.HH.COVID)+1] <- list(a)
    names(robustnesslist2.HH.COVID)[length(robustnesslist2.HH.COVID)] <- paste(as.character(predictor.number), " dropped")
