...
var.dependent = "14 days hospitalization incidence" #defining the dependent variable
...
pool<-synthdata[which(synthdata$UnitNumeric%in%c(2:13,15:17) &synthdata$DateNumeric==1),c(1,4)]
treated.unit=pool$UnitNumeric[pool$Name=="Hamburg"]
print(pool)
...
write_xlsx(data.frame(as.vector(rownames(synth.tables.orig$tab.pred)[c(4:6,2,3,1)]), #row names, the vector c(4:6,2,3,1) correct the order of rows to fit the final text
           ...
           "Figures and Tables/Table 7.xlsx") 
# Table 8####
write_xlsx(data.frame(synth.tables.orig$tab.w), 
           "Figures and Tables/Table 8.xlsx") 

# Figure 12: treated and synthetic####
png(filename="Figures and Tables/Figure 12 - HH Hospitalization Incidence _ Synthetic Control.png", 
    units="px", 
    width=1980, 
    height=1080, 
    pointsize=25, 
...
plot(synthdata$Date[plot.period],dataprep.out.orig$Y1plot,type="l",lwd=4,
     xlab = "", ylab = "14-days hospitalization incidence",
     ylim = c(0,30),
     cex.lab = 1.5)
...
legend(x="topright", legend=c("Hamburg","Synthetic Hamburg"),
       lty = c(1,2), lwd = c(4,4), cex = 1.5)
dev.off()
# Figure 16 - Appendix: robustness checks - dropping donor unit####
png(filename="Figures and Tables/Figure 16 - HH Hospitalization Incidence _ Leave-one-out Donors.png", 
    units="px", 
    width=1980, 
    height=1080, 
    pointsize=25, 
...
abline(v = synthdata$Date[synthdata$UnitNumeric==1 
                          & synthdata$Date==as.Date("2022-04-03")])
legend(x="topright", legend=c("Hamburg","Synthetic Hamburg", "leave-one-out estimates"),
       lty = c(1,2,1), 
       cex = 1.5,
       lwd = c(4,4,1) )
...
png(filename="Figures and Tables/Figure 17 - HH Hospitalization Incidence _ Leave-one-out Predictors.png", 
    units="px", 
    width=1980, 
    height=1080, 
    pointsize=25, 
...
robustnesslist2.HH.hospitalizations<-list() #creating a list that contains all synthetic control estimations conducted for this robustness test. This list won't be used for further results, but has a purpose for double-checking results
a<-list(dataprep.out.orig$Y1plot,dataprep.out.orig$Y0plot,synth.out.orig,synth.tables.orig)
names(a)<-c("Y1","Y0","synth.out","synth.tables")
robustnesslist2.HH.hospitalizations[length(robustnesslist2.HH.hospitalizations)+1] <- list(a)
names(robustnesslist2.HH.hospitalizations)[length(robustnesslist2.HH.hospitalizations)] <- "original treatment unit"
...
for (predictor.number in c(1:length(var.special.predictors))) {
  ...
  names(a)<-c("Y1","Y0","synth.out","synth.tables")
  robustnesslist2.HH.hospitalizations[length(robustnesslist2.HH.hospitalizations)+1] <- list(a)
  names(robustnesslist2.HH.hospitalizations)[length(robustnesslist2.HH.hospitalizations)] <- paste(as.character(predictor.number), " dropped")
...
# Figure 18 - Appendix: placebo tests####

png(filename="Figures and Tables/Figure 18 - HH Hospitalization Incidence _ Placebo Tests.png", 
    units="px", 
    width=1980, 
    height=1080, 
    pointsize=25, 
...
placebolistHH.hosp<-list()
a<-list(dataprep.out.orig$Y1plot,dataprep.out.orig$Y0plot,synth.out.orig,synth.tables.orig)
names(a)<-c("Y1","Y0","synth.out","synth.tables")
placebolistHH.hosp[length(placebolistHH.hosp)+1] <- list(a)
names(placebolistHH.hosp)[length(placebolistHH.hosp)] <- "original treatment unit"
...
for (unit in pool$UnitNumeric) {
  ...
  names(a)<-c("Y1","Y0","synth.out","synth.tables")
  placebolistHH.hosp[length(placebolistHH.hosp)+1] <- list(a)
  names(placebolistHH.hosp)[length(placebolistHH.hosp)] <- unit
  
  #skip, if MSPE exceeds restriction
  if (synth.tables$tab.loss[2]>MSPErestrict*placebolistHH.hosp[[1]][[4]]$tab.loss[2])
  {next}
...
i=1
PostPre<-c()
while (i<= length(placebolistHH.hosp)) { #this loop uses the data stored in placebolistHH.hosp to calculate the Post/Pre Interventions MSPEs for all synthetic controls (subject to the constraint, that the pretreatment MSPE is not 'MSPErestrict' times bigger than the original MSPE)
  PostPre[length(PostPre)+1]<-
    (sum((placebolistHH.hosp[[i]][[1]][c((1+length(optimization.period)):length(plot.period)),]- #Post-Intervention MSPE
            placebolistHH.hosp[[i]][[2]][c((1+length(optimization.period)):length(plot.period)),] %*% placebolistHH.hosp[[i]][[3]]$solution.w)^2#Post-Intervention MSPE
    )/(length(plot.period)-length(optimization.period)))/ 
    (sum((placebolistHH.hosp[[i]][[1]][1:length(optimization.period),] - #Pre-Intervention MSPE
            placebolistHH.hosp[[i]][[2]][1:length(optimization.period),] %*% placebolistHH.hosp[[i]][[3]]$solution.w)^2
...
png(filename="Figures and Tables/Figure 15 - HH Hospitalizations Incidence _ PrePost MSPE.png", 
    units="px", 
    width=1080, 
    height=1080, 
    pointsize=25, 
