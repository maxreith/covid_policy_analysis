# analysis_422_hh_hosp.R
# Section 4.2.2: Hamburg Hospitalization Incidence Level Analysis
#
# This script runs synthetic control analysis for Hamburg
# using 14-day hospitalization incidence (level, not growth rate)
# as the dependent variable. Donor pool excludes MV.
...
main <- function(save_outputs = TRUE, verbose = FALSE, use_parquet = TRUE) {
  ...
  pool <- get_hamburg_hosp_pool(synthdata)
  ...
  results$table7 <- format_predictor_table(results$synth_tables)
  results$table8 <- format_weights_table(results$synth_tables)
...
get_analysis_config <- function() {
  list(
    ...
    figure_prefix = "HH Hospitalization Incidence"
  )
}
...
save_analysis_outputs <- function(results, synthdata, config) {
  ...
  arrow::write_parquet(
    results$table7,
    file.path(output_dir, "table7_hh_hosp_predictors.parquet")
  )
  arrow::write_parquet(
    results$table8,
    file.path(output_dir, "table8_hh_hosp_weights.parquet")
  )
}
