# test-421-mv-hosp.R
# Tests for Section 4.2.1: MV Hospitalization Analysis
# Verifies refactored code produces identical results to original

BASELINE_NAME <- "section_421"

test_that("Section 4.2.1 baseline exists or skip", {
...
test_that("Section 4.2.1 main synth dataprep output has expected structure", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 main synth output has expected structure", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 synth tables have expected structure", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 placebo list has expected length", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 post/pre MSPE ratios have expected length", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 robustness donor list exists", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 robustness predictor list exists", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 table5 has expected structure", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)

  expect_true(!is.null(baseline$table5))
  expect_equal(nrow(baseline$table5), 6L)
  expect_true("predictor" %in% names(baseline$table5))
  expect_true("v_weight" %in% names(baseline$table5))
})

test_that("Section 4.2.1 table6 has expected structure", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)

  expect_true(!is.null(baseline$table6))
  n_donors <- nrow(baseline$pool) - 1
  expect_equal(nrow(baseline$table6), n_donors)
})

test_that("Section 4.2.1 treated unit is correct", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 donor pool is correct", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 dependent variable is hospitalization growth rate", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)

  expect_equal(baseline$var_dependent, "14 days hospitalization incidence growth rate")
})

test_that("Section 4.2.1 predictors use hospitalization variable", {
  skip_if_no_baseline(BASELINE_NAME)
  baseline <- load_baseline(BASELINE_NAME)

  hosp_predictors <- sapply(baseline$var_special_predictors, function(x) {
    grepl("hospitalization", x[[1]])
  })
  expect_true(sum(hosp_predictors) == 3L)
})

test_that("Section 4.2.1 placebo results can be compared", {
  skip_if_no_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 donor robustness results can be compared", {
  skip_if_no_baseline(BASELINE_NAME)
...
test_that("Section 4.2.1 predictor robustness results can be compared", {
  skip_if_no_baseline(BASELINE_NAME)
  skip_if_not_full_test()
  baseline <- load_baseline(BASELINE_NAME)
